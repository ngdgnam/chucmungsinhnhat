...
const DATA_FILE = path.resolve("Api", "Data", "Image", "Instagram.json");
...
module.exports = {
  name: "insimg",
  description: "Random ảnh Instagram",
  role: 0,
  cooldown: 5,
  group: "image",
  aliases: ["instagram"],
  ...
  async run({ message, api }) {
    ...
    try {
      ...
      const url = pick(urls);
      const dir = path.resolve("Data", "Cache", "Instagram");
      const ts = Date.now();
      const ext = (() => { try { const u = new URL(url); return path.extname(u.pathname) || ".jpg"; } catch { return ".jpg"; } })();
      const filePath = path.join(dir, `ins_${ts}${ext}`);
      await downloadStream(url, filePath);
      await api.sendMessage({ msg: "Instagram đây ✨", attachments: [filePath], ttl: 30000 }, threadId, threadType);
